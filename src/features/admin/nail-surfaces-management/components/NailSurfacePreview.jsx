...
export function NailSurfacePreview({ surface, compact = false }) {
  ...
  return (
    <div className={compact ? "inline-flex items-center gap-3" : "rounded-[24px] border border-[#f7d7e5] bg-white p-4"}>
      <div
        className={compact
          ? "relative rounded-[18px] bg-[radial-gradient(circle_at_top,#fff6fb_0%,#fff2f8_100%)] p-0 w-[60px] h-[80px] overflow-hidden"
          : "relative rounded-[20px] bg-[radial-gradient(circle_at_top,#fff6fb_0%,#fff0f7_55%,#fff8fb_100%)] p-0 w-full h-[240px] overflow-hidden"
        }
      >
        <Canvas camera={{ position: [0, 0, compact ? 3.5 : 5], fov: 45 }}>
          ...
          {Array.from({ length: numNails }).map((_, index) => {
            ...
            const yOffset = 0; // Align straight horizontally
            const zRotation = 0; // Point straight up
