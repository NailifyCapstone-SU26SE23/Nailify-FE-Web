                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-[10px] text-[#9E8497] mt-0.5 font-mono">#{tx.description || tx.orderCode}</p>
                        </div>
                      ...
                      <div className="mt-1 pt-2 border-t border-[#F3E2EC] border-dashed space-y-1">
                        {tx.createdAt && (
                          <div className="flex justify-between items-center text-[10px]">
                            ...
                            <span className="font-medium text-[#2B182B]">{dayjs(tx.createdAt).format('MMM DD, YYYY h:mm A')}</span>
                          </div>
                        )}
                        {tx.paidAt && (
                          <div className="flex justify-between items-center text-[10px]">
                            <span className="text-[#9E8497] font-medium">{language === "vi" ? "Thanh toán lúc" : "Paid At"}</span>
                            <span className="font-medium text-[#059669]">{dayjs(tx.paidAt).format('MMM DD, YYYY h:mm A')}</span>
                          </div>
                        )}
                        {!tx.paidAt && tx.expiresAt && (
                          <div className="flex justify-between items-center text-[10px]">
                            <span className="text-[#9E8497] font-medium">{language === "vi" ? "Hết hạn lúc" : "Expires At"}</span>
                            <span className="font-medium text-[#E11D48]">{dayjs(tx.expiresAt).format('MMM DD, YYYY h:mm A')}</span>
                          </div>
                        )}
                        <div className="flex justify-between items-center text-[10px]">
