...
function getSurfaceIcon(name: string): string {
...
export function BuilderControls() {
  ...
  const updateGradientPreview = useCallback(() => {
    ...
    const c1 = (document.getElementById('gradient-color-1') as HTMLInputElement)?.value ?? '#FF4081';
    const c2 = (document.getElementById('gradient-color-2') as HTMLInputElement)?.value ?? '#FFFFFF';
    const c3 = (document.getElementById('gradient-color-3') as HTMLInputElement)?.value ?? '#000000';
    const gType = (document.getElementById('gradient-type') as HTMLSelectElement)?.value ?? 'linear';
    const checkedRadio = document.querySelector<HTMLInputElement>('input[name="stop-count"]:checked');
    const sc = checkedRadio ? parseInt(checkedRadio.value, 10) : 2;

    let gradientCSS: string;
    if (gType === 'radial') {
      gradientCSS = sc === 3 ? `radial-gradient(circle, ${c1}, ${c2}, ${c3})` : `radial-gradient(circle, ${c1}, ${c2})`;
    } else if (gType === 'horizontal') {
      gradientCSS = sc === 3 ? `linear-gradient(90deg, ${c1}, ${c2}, ${c3})` : `linear-gradient(90deg, ${c1}, ${c2})`;
    } else {
      gradientCSS = sc === 3 ? `linear-gradient(180deg, ${c1}, ${c2}, ${c3})` : `linear-gradient(180deg, ${c1}, ${c2})`;
    }
  ...
  const handleModeChange = useCallback(
    (mode: ColorMode) => {
      ...
      const enableGradientCheckbox = document.getElementById('enable-gradient') as HTMLInputElement | null;
      if (!enableGradientCheckbox) return;

      if (mode === 'gradient') {
        if (!enableGradientCheckbox.checked) {
          enableGradientCheckbox.checked = true;
          enableGradientCheckbox.dispatchEvent(new Event('change', { bubbles: true }));
        }
  ...
  useEffect(() => {
    ...
    const ids = ['gradient-color-1', 'gradient-color-2', 'gradient-color-3', 'gradient-type'];
    ids.forEach((id) => document.getElementById(id)?.addEventListener('input', handler));
    document.querySelectorAll<HTMLInputElement>('input[name="stop-count"]').forEach((radio) =>
    ...
    return () => {
      ids.forEach((id) => document.getElementById(id)?.removeEventListener('input', handler));
      document.querySelectorAll<HTMLInputElement>('input[name="stop-count"]').forEach((radio) =>
    ...
    <div className="builder-controls-grid">
      <section className="builder-panel builder-style-panel">
        ...
        <div className="paint-studio">
          ...
          {/* ─── Solid Color Mode ─── */}
          <div className={`paint-panel ${colorMode === 'solid' ? 'active' : ''}`}>
            <div className="color-palette">
              ...
              <input type="color" id="custom-color" defaultValue="#FF4081" title="Custom Color" />
            </div>
          </div>

          {/* ─── Gradient Mode ─── */}
          <div className={`paint-panel ${colorMode === 'gradient' ? 'active' : ''}`}>
            {/* Hidden checkbox — preserved for handLandmarkerTask.ts event handler */}
            <input
              type="checkbox"
              id="enable-gradient"
            ...
            <div className="gradient-presets-section">
              ...
              <div className="gradient-presets-grid">
                {[
                  { name: 'French Ombre', c1: '#FFB6C1', c2: '#FFFFFF', c3: '#FFFFFF', stops: 2, type: 'linear', bg: 'linear-gradient(180deg, #FFB6C1, #FFFFFF)' },
                  { name: 'Sunset Glow', c1: '#FF4081', c2: '#FFD700', c3: '#FFD700', stops: 2, type: 'linear', bg: 'linear-gradient(180deg, #FF4081, #FFD700)' },
                  { name: 'Berry Sorbet', c1: '#E91E63', c2: '#FFB6C1', c3: '#FFFFFF', stops: 3, type: 'linear', bg: 'linear-gradient(180deg, #E91E63, #FFB6C1, #FFFFFF)' },
                  { name: 'Ocean Mist', c1: '#0D47A1', c2: '#42A5F5', c3: '#42A5F5', stops: 2, type: 'linear', bg: 'linear-gradient(180deg, #0D47A1, #42A5F5)' },
                ].map((preset) => (
                  <button
                    key={preset.name}
                    type="button"
                    ...
                    title={preset.name}
                    onClick={() => {
                      const input1 = document.getElementById('gradient-color-1') as HTMLInputElement | null;
                      const input2 = document.getElementById('gradient-color-2') as HTMLInputElement | null;
                      const input3 = document.getElementById('gradient-color-3') as HTMLInputElement | null;
                      const typeSelect = document.getElementById('gradient-type') as HTMLSelectElement | null;
                      const radio = document.querySelector(`input[name="stop-count"][value="${preset.stops}"]`) as HTMLInputElement | null;
            ...
            <div id="gradient-controls" className="gradient-controls" style={{ display: colorMode === 'gradient' ? 'flex' : 'none' }}>
              {/* Color count toggle */}
              <div className="gradient-stop-count">
                ...
                <div className="stop-count-selector">
                  <label className="stop-count-chip">
                    <input type="radio" name="stop-count" value="2" defaultChecked /> 2 Colors (Ombre)
                  </label>
                  <label className="stop-count-chip">
                    <input type="radio" name="stop-count" value="3" /> 3 Colors (Trio)
                  </label>
                </div>
              </div>
              ...
              <div className="gradient-stop-group">
                <label className="gradient-stop-label">
                  ...
                  <input type="color" id="gradient-color-1" defaultValue="#FF4081" title="Base Color" />
                </label>
                {stopCount === 3 ? (
                  <>
                    <label className="gradient-stop-label">
                    ...
                    <label className="gradient-stop-label">
                      ...
                      <input type="color" id="gradient-color-3" defaultValue="#000000" title="Tip Color" />
                    </label>
                  </>
                ) : (
                  <>
                    <label className="gradient-stop-label">
                      <span className="gradient-stop-title">Tip Color</span>
                      <input type="color" id="gradient-color-2" defaultValue="#FFFFFF" title="Tip Color" />
                    </label>
                    {/* Render gradient-color-3 in a hidden container to prevent script error or styling clash */}
                    <div style={{ display: 'none' }}>
                      <input type="color" id="gradient-color-3" defaultValue="#000000" />
                    </div>
                  </>
                )}
              </div>
            ...
            {nailSurfaces.length > 0 ? (
              nailSurfaces.map((surface, index) => (
                <button
                  ...
                  type="button"
                >
                  <span className="material-btn-icon">{getSurfaceIcon(surface.name)}</span>
